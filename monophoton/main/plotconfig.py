...
minor = ['zllg-130-o', 'zllg-300-o', 'gg-40', 'gg-80'] # + wlnu # 'ttg', 'tg',
...
#        config.addBkg('top', '#gamma + t(t)', samples = ['ttg', 'tg'], color = ROOT.TColor.GetColor(0x55, 0x44, 0xff), cut = 'photons.matchedGenId[0] == -22')
        ...
        config.fullSelection = 't1Met.pt > 100. && t1Met.minJetDPhi > 1.'

        config.addSig('dph', 'Dark Photon', samples = ['dphv-*'])
        ...
        config.addPlot('mtFullSel', 'M_{T}^{#gamma}', 'photons.mt[0]', (10, 0., 300.), unit = 'GeV', applyFullSel = True, overflow = True, sensitive = True, blind = 'full')
...
#        config.findGroup('efake').variations.append(Variation('egfakerate', reweight = 'egfakerate'))
    ...
    elif confName == 'gghg' or confName == 'gghgBlind':
        ...
        config.baseline = baseSel.replace(baseSels['met170'], 't1Met.pt > 0.').replace(baseSels['photonPtOverMet1.4'], '1') + ' && photons.mt[0] < 300.' 
        ...
        config.addBkg('minor', '#gamma#gamma, Z#rightarrowll+#gamma', samples = ['zllg-130-o', 'zllg-300-o'], color = ROOT.TColor.GetColor(0xbb, 0x66, 0xff))
        config.addBkg('wjets', 'W(#mu,#tau) + jets', samples = wlnun, color = ROOT.TColor.GetColor(0x22, 0x22, 0x22))
        # config.addBkg('vvg', 'VV#gamma', samples = ['ww', 'wz', 'zz'], color = ROOT.TColor.GetColor(0xff, 0x44, 0x99))
        config.addBkg('top', 't#bar{t}#gamma/t#gamma', samples = ['ttg', 'tg'], color = ROOT.TColor.GetColor(0x55, 0x44, 0xff))
        config.addBkg('gjets', '#gamma + jets', samples = gj04, color = ROOT.TColor.GetColor(0xff, 0xaa, 0xcc))
        ...
        config.addPlot('recoil', 'E_{T}^{miss}', 't1Met.pt', combinedFitPtBinning, unit = 'GeV', overflow = True, sensitive = True)
        config.addPlot('recoilScan', 'E_{T}^{miss}', 't1Met.pt', [0. + 25. * x for x in range(13)], unit = 'GeV', overflow = True, sensitive = True)
        config.addPlot('mtPhoMet', 'M_{T#gamma}', 'photons.mt[0]', gghMtBinning, unit = 'GeV', overflow = True, sensitive = True)
        config.addPlot('mtPhoMetFullDPhi', 'M_{T#gamma}', 'photons.mt[0]', gghMtBinning, unit = 'GeV', overflow = True, applyBaseline = False, cut = noDPhiPhoton, sensitive = True)
        config.addPlot('phoPtHighMet', 'E_{T}^{#gamma}', 'photons.scRawPt[0]', combinedFitPtBinning, unit = 'GeV', overflow = True, sensitive = True)
        config.addPlot('phoPtScan', 'E_{T}^{#gamma}', 'photons.scRawPt[0]', [175. + 25. * x for x in range(14)], unit = 'GeV', overflow = True, sensitive = True)
        config.addPlot('phoEta', '#eta^{#gamma}', 'photons.eta_[0]', (20, -1.5, 1.5))
        config.addPlot('phoPhi', '#phi^{#gamma}', 'photons.phi_[0]', (20, -math.pi, math.pi))
        config.addPlot('nphotons', 'N_{#gamma}', 'photons.size', (4, 0., 4.))
        ...
        config.addPlot('dPhiPhoJetMin', 'min#Delta#phi(#gamma, j)', 'photons.minJetDPhi[0]', (14, 0., 3.50), overflow = True)
        config.addPlot('njets', 'N_{jet}', 'Sum$(jets.pt_ > 30.)', (6, 0., 6.), ymax = 5.e+3, sensitive = True)
        config.addPlot('njetsHighPt', 'N_{jet} (p_{T} > 100 GeV)', 'Sum$(jets.pt_ > 30.)', (10, 0., 10.)) #, cut = 'jets.pt_ > 100.')
        config.addPlot('jetPt', 'p_{T}^{jet}', 'jets.pt_', [0., 100., 200., 300., 400., 600., 1000.], unit = 'GeV', cut = 'jets.pt_ > 30', overflow = True)
        config.addPlot('phoPtOverMet', 'E_{T}^{#gamma}/E_{T}^{miss}', 'photons.scRawPt[0] / t1Met.pt', (30, 0., 3.))
        config.addPlot('phoPtOverJetPt', 'E_{T}^{#gamma}/p_{T}^{jet}', 'photons.scRawPt[0] / jets.pt_[0]', (20, 0., 10.))
        config.addPlot('metSignif', 'E_{T}^{miss} Significance', 't1Met.pt / TMath::Sqrt(t1Met.sumETRaw)', (15, 0., 30.), sensitive = True)
        ...
        for plot in list(config.plots):
            if plot.name not in ['recoil', 'recoil']:
                config.plots.append(plot.clone(plot.name + 'HighMet', applyFullSel = True))
        ...
        config.baseline = baseSel.replace('minJetDPhi', 'realMinJetDPhi') + ' && photons.mt[0] < 300. && dimu.oppSign && dimu.mass[0] > 60. && dimu.mass[0] < 120.'
        config.fullSelection = ''
        ...
        config.addPlot('recoil', 'Recoil', 't1Met.pt', combinedFitPtBinning, unit = 'GeV', overflow = True)
        config.addPlot('recoilScan', 'Recoil', 't1Met.pt', [175. + 25. * x for x in range(14)], unit = 'GeV', overflow = True)
        config.addPlot('mtPhoMet', 'M_{T#gamma}', 'photons.mt[0]', gghMtBinning, unit = 'GeV', overflow = True)
        config.addPlot('mtPhoMetFullDPhi', 'M_{T#gamma}', 'photons.mt[0]', gghMtBinning, unit = 'GeV', overflow = True, applyBaseline = False, cut = noDPhiPhoton)
        config.addPlot('phoPtHighMet', 'E_{T}^{#gamma}', 'photons.scRawPt[0]', combinedFitPtBinning, unit = 'GeV', overflow = True, applyFullSel = True)
        config.addPlot('phoPtScan', 'E_{T}^{#gamma}', 'photons.scRawPt[0]', [175. + 25. * x for x in range(14)], unit = 'GeV', overflow = True)
        config.addPlot('phoEta', '#eta^{#gamma}', 'photons.eta_[0]', (10, -1.5, 1.5))
        config.addPlot('phoPhi', '#phi^{#gamma}', 'photons.phi_[0]', (10, -math.pi, math.pi))
...
#        Standard MC systematic variations
        ...
        config.baseline = baseSel.replace('minJetDPhi', 'realMinJetDPhi') + ' && photons.mt[0] < 300. && diel.oppSign && diel.mass[0] > 60. && diel.mass[0] < 120.'
        config.fullSelection = ''
...
#        MinIf$() somehow returns 0 when there is only one jet
        ...
        config.baseline = baseSel.replace('minJetDPhi', 'realMinJetDPhi') + ' && photons.mt[0] < 300. && ' + mt + ' < 160.' 
        config.fullSelection = ''
...
#        MinIf$() somehow returns 0 when there is only one jet
        ...
        config.baseline = baseSel.replace('minJetDPhi', 'realMinJetDPhi') + ' && photons.mt[0] < 300. && ' + mt + ' < 160. && t1Met.realMet > 50.' 
        config.fullSelection = ''
