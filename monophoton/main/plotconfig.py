...
muonData = ['smu-16b-r', 'smu-16c-r', 'smu-16d-r', 'smu-16e-r', 'smu-16f-r', 'smu-16g-r', 'smu-16h']
...
#            GroupSpec('dmafs', 'DM A', samples = ['dmafs-*'])
        ...
        config.bkgGroups = [
            GroupSpec('minor', 'Minor SM', samples = ['ttg', 'tg', 'zllg-130'], color = ROOT.TColor.GetColor(0x55, 0x44, 0xff)), #  + wlnu + ['gg-80']
        ...
        config.variables = [
            ...
            VariableDef('phoPtScan', 'E_{T}^{#gamma}', 'photons.scRawPt[0]', [175.] + [200. + 10. * x for x in range(40)], unit = 'GeV', overflow = True, applyBaseline = False, cut = 'photons.scRawPt[0] > 175. && t1Met.met > 100 && t1Met.photonDPhi > 2. && t1Met.minJetDPhi > 0.5'),
        ...
        config.baseline = baseSel.replace('minJetDPhi', 'realMinJetDPhi') + ' && dimu.oppSign && dimu.mass[0] > 60. && dimu.mass[0] < 120.' # && muons.pt[1] > 20.'  # met is the recoil (Operator LeptonRecoil)
        config.fullSelection = ''
        config.bkgGroups = [
            # GroupSpec('vvg', 'VV#gamma', samples = ['ww', 'wz', 'zz'], color = ROOT.TColor.GetColor(0xff, 0x44, 0x99)), # no events pass selection, screws up fit
        ...
        config.variables = [
            ...
            VariableDef('dimumass', 'M_{#mu#mu}', 'z.mass[0]', (12, 60., 120.), unit = 'GeV', overflow = True, applyBaseline = False, cut = baseSel.replace('minJetDPhi', 'realMinJetDPhi') + ' && z.oppSign'),
            ...
            VariableDef('nMu', 'N_{#mu}', 'muons.size', [2, 3, 4, 5]),
            ...
            VariableDef('mu0Iso', 'I^{leading #mu}_{comb.}/p_{T}', 'muons.combRelIso[0]', (20, 0., 0.4), overflow = True),
            ...
            VariableDef('mu1Iso', 'I^{trailing #mu}_{comb.}/p_{T}', 'muons.combRelIso[1]', (20, 0., 0.4), overflow = True),
            ...
            VariableDef('dPhiJetMetMin', 'min#Delta#phi(E_{T}^{miss}, j)', 't1Met.realMinJetDPhi', (10, 0., math.pi), applyBaseline = False, cut = mass + ' > 75. && ' + mass + ' < 105. && photons.scRawPt[0] > 175. && t1Met.met > 170. && t1Met.photonDPhi > 2. && jets.size != 0'),
        ...
        config.baseline = baseSel.replace('minJetDPhi', 'realMinJetDPhi') + ' && diel.oppSign && diel.mass[0] > 60. && diel.mass[0] < 120.' # && electrons.pt[1] > 20.' # met is the recoil (Operator LeptonRecoil)
        config.fullSelection = ''
        config.bkgGroups = [
            # GroupSpec('vvg', 'VV#gamma', samples = ['ww', 'wz', 'zz'], color = ROOT.TColor.GetColor(0xff, 0x44, 0x99)),
        ...
        config.variables = [
            ...
            VariableDef('phoPhi', '#phi^{#gamma}', 'photons.phi[0]', (10, -math.pi, math.pi)),
            # VariableDef('dPhiPhoMet', '#Delta#phi(#gamma, E_{T}^{miss})', 't1Met.realPhotonDPhi', (10, 0., math.pi)),
            VariableDef('dPhiPhoRecoil', '#Delta#phi(#gamma, U)', 't1Met.photonDPhi', (10, 0., math.pi), applyBaseline = False, cut = mass + ' > 75. && ' + mass + ' < 105. && photons.scRawPt[0] > 175. && t1Met.met > 170. && t1Met.realMinJetDPhi > 0.5'),
            VariableDef('dRPhoEl', '#DeltaR(#gamma, e)_{min}', 'TMath::Sqrt(TMath::Min(%s, %s))' % (dR2_00, dR2_01), (10, 0., 4.)),
            VariableDef('dielmass', 'M_{ee}', 'z.mass[0]', (12, 60., 120.), unit = 'GeV', overflow = True, applyBaseline = False, cut = baseSel.replace('minJetDPhi', 'realMinJetDPhi') + ' && z.oppSign'),
            VariableDef('zPt', 'p_{T}^{Z}', 'z.pt[0]', combinedFitPtBinning, unit = 'GeV'),
            VariableDef('zEta', '#eta_{Z}', 'z.eta[0]', (10, -5., 5.)),
            VariableDef('zPhi', '#phi_{Z}', 'z.phi[0]', (10, -math.pi, math.pi)),
            VariableDef('nEl', 'N_{e}', 'electrons.size', [2, 3, 4, 5]),
        ...
        config.variables = [
            ...
            VariableDef('muIso', 'I^{#mu}_{comb.}/p_{T}', 'muons.combRelIso[0]', (20, 0., 0.4), overflow = True),
            VariableDef('njets', 'N_{jet}', 'jets.size', (6, 0., 6.)),
...
#                config.variables.remove(variable)
        ...
        config = PlotConfig('monoph', photonData)
        config.baseline = 'photons.scRawPt[0] > 175. && t1Met.minJetDPhi > 0.5 && t1Met.photonDPhi > 2. && t1Met.met > 100. && t1Met.met < 150.'
        config.fullSelection = ''
        config.sigGroups = [
            GroupSpec('dmv', 'DM V', samples = ['dmv-500-1', 'dmv-1000-1', 'dmv-2000-1']),
            ]            
        ...
        config.variables = [
            VariableDef('met', 'E_{T}^{miss}', 't1Met.met', [100. + 10. * x for x in range(6)], unit = 'GeV', overflow = True),
            VariableDef('mtPhoMet', 'M_{T#gamma}', mtPhoMet, (22, 200., 1300.), unit = 'GeV', overflow = True),
            VariableDef('phoPt', 'E_{T}^{#gamma}', 'photons.scRawPt[0]', [175.] + [180. + 10. * x for x in range(12)] + [300., 350., 400., 450.] + [500. + 100. * x for x in range(6)], unit = 'GeV', overflow = True),
        ...
        config.variables = [
            ...
            VariableDef('jetPt', 'E_{T}^{j}', 'jets.pt[0]', [175. + 25. * x for x in range(5)] + [300., 350., 400., 450.] + [500. + 100. * x for x in range(6)], unit = 'GeV', overflow = True),
            VariableDef('metPhi', '#phi(E_{T}^{miss})', 't1Met.phi', (20, -math.pi, math.pi)),
            VariableDef('dPhiJetMet', '#Delta#phi(E_{T}^{miss}, j)', 'TMath::Abs(TVector2::Phi_mpi_pi(jets.phi - t1Met.phi))', (30, 0., math.pi), cut = 'jets.pt > 30.'),
            VariableDef('dPhiJetMetMin', 'min#Delta#phi(E_{T}^{miss}, j)', 't1Met.minJetDPhi', (30, 0., math.pi), overflow = True),
            VariableDef('njets', 'N_{jet}', 'jets.size', (10, 0., 10.)),
        ...
        config.fullSelection = 'TMath::Abs(TVector2::Phi_mpi_pi(jets.phi[0] - t1Met.phi)) < 3.0'
        config.bkgGroups = [
        ...
        config.variables = [
            ...
            VariableDef('phoPt', 'E_{T}^{#gamma}', 'photons.scRawPt[0]', [175. + 15. * x for x in range(20)], unit = 'GeV', overflow = True),
            VariableDef('phoPtLowSx', 'E_{T}^{#gamma}', 'photons.scRawPt[0]', [175. + 15. * x for x in range(20)], cut = 'photons.e4[0] / photons.emax[0] > 0.5', unit = 'GeV', overflow = True),
            ...
            VariableDef('phoPtHighMet', 'E_{T}^{#gamma}', 'photons.scRawPt[0]', [175. + 15. * x for x in range(20)], cut = 't1Met.met > 170.', unit = 'GeV', overflow = True),
            VariableDef('phoEta', '#eta^{#gamma}', 'photons.eta[0]', (20, -1.5, 1.5)),
            VariableDef('phoEtaLowSx', '#eta^{#gamma}', 'photons.eta[0]', (20, -1.5, 1.5), cut = 'photons.e4[0] / photons.emax[0] > 0.5'),
            VariableDef('phoEta2Jets', '#eta^{#gamma}', 'photons.eta[0]', (20, -1.5, 1.5), cut = 'jets.size == 2'),
...
#            VariableDef('dPhiPhoMet', '#Delta#phi(#gamma, E_{T}^{miss})', 'TVector2::Phi_mpi_pi(photons.phi[0] - t1Met.phi)', (20, -math.pi, math.pi), cut = 'photons.scRawPt[0] > 175. && t1Met.met > 170. && t1Met.minJetDPhi > 0.5', applyBaseline = False, logy = False),
            ...
            VariableDef('dPhiJetMetMin', 'min#Delta#phi(E_{T}^{miss}, j)', 't1Met.minJetDPhi', (30, 0., math.pi), overflow = True),
            VariableDef('mtPhoMet', 'M_{T#gamma}', 'photons.mt[0]', (30, 0., 300.), unit = 'GeV', logy = False),
            VariableDef('mtPhoMetHighSx', 'M_{T#gamma}', 'photons.mt[0]', (30, 0., 300.), cut = 'photons.e4[0] / photons.emax[0] < 0.5', unit = 'GeV'),
            VariableDef('metPhi', '#phi(E_{T}^{miss})', 't1Met.phi', (20, -math.pi, math.pi), logy = False),
            VariableDef('njets', 'N_{jet}', 'jets.size', (6, 0., 6.)),
            VariableDef('jetPt', 'p_{T}^{j1}', 'jets.pt[0]', (30, 30., 800.), cut = 'jets.size != 0', unit = 'GeV'),
            ...
            VariableDef('jetEta', '#eta^{j1}', 'jets.eta[0]', (30, -5., 5.), cut = 'jets.size != 0'),
            VariableDef('jet2Pt', 'p_{T}^{j2}', 'jets.pt[1]', (30, 30., 800.), cut = 'jets.size > 1', unit = 'GeV'),
            VariableDef('jet2Eta', '#eta^{j2}', 'jets.eta[1]', (30, -5., 5.), cut = 'jets.size > 1'),
            VariableDef('r9', 'R_{9}', 'photons.r9', (50, 0.5, 1.)),
            VariableDef('r9HighSx', 'R_{9}', 'photons.r9', (50, 0.5, 1.), cut = 'photons.e4[0] / photons.emax[0] < 0.5'),
...
#                config.variables.remove(variable)
        ...
        config.fullSelection = 't1Met.met > 75.'
        config.bkgGroups = [
        ...
        config.variables = [ 
            ...
            VariableDef('dPhiJetMet', '#Delta#phi(E_{T}^{miss}, jet)', 'TMath::Abs(TVector2::Phi_mpi_pi(t1Met.phi - jets.phi))', (15, 0., math.pi), applyBaseline = False, cut = 'z.mass > 81. && z.mass < 101. && z.oppSign == 1 && jets.size == 1 && jets.pt[0] > 100. && t1Met.met > 50. && t1Met.minJetDPhi > 0.5'),
...
#                config.variables.remove(variable)
        ...
        config.baseline = 'photons.scRawPt[0] > 175. && photons.chIsoMax > 0.441 && t1Met.minJetDPhi > 0.5 && t1Met.photonDPhi > 2. && t1Met.met > 170.'
        config.fullSelection = 't1Met.met > 170.'
        config.bkgGroups = [
            GroupSpec('minor', 'Minor SM', samples = ['ttg', 'tg', 'zllg-130'] + wlnu + ['gg-80'], color = ROOT.TColor.GetColor(0x55, 0x44, 0xff)),
            GroupSpec('gjets', '#gamma + jets', samples = gj04, color = ROOT.TColor.GetColor(0xff, 0xaa, 0xcc)),
