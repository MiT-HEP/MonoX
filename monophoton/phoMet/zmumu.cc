...
zmumu(TTree* _input, char const* _outputName, double _sampleWeight = 1., TH1* _puweight = 0)
{
  ...
  while (_input->GetEntry(iEntry++) > 0) {
    ...
    for (unsigned iTag(0); iTag != muons.size(); ++iTag) {
      auto& tag(muons[iTag]);
      if ( !(tag.tight && tag.pt > 30.))
	continue;

      //printf("Tag muon found\n");
      
      for (unsigned iProbe(iTag); iProbe != muons.size(); ++iProbe) {
	auto& probe(muons[iProbe]);
	if ( !(probe.loose && probe.pt > 20.))
	  continue;

	//printf("Loose muon found for probe\n");
