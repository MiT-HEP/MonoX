...
void mergeData() {
  ...
  TF1 *ZmmBin   = (TF1*) corrections->Get("fcn_mu_u1_Data_Zmm");
  TF1 *GJetsBin = (TF1*) corrections->Get("fcn_mu_u1_Data_gjets");
  ...
  for (Int_t iEntry = 0; iEntry != nentries; ++iEntry) {
    ...
    if (iEntry % 10000 == 0)
      std::cout << "Processing DY: " << float(iEntry) * 100/float(nentries) << "%" << std::endl;
  ...
  for (Int_t iEntry = 0; iEntry != nentries; ++iEntry) {
    ...
    if (iEntry % 10000 == 0)
      std::cout << "Processing GJets: " << float(iEntry) * 100/float(nentries) << "%" << std::endl;
    ...
    mergedTree->met = gjetReader->u_magZ;
    mergedTree->metPhi = gjetReader->u_phiZ;
    mergedTree->trueMet = gjetReader->trueMet;
    mergedTree->trueMetPhi = gjetReader->trueMetPhi;
    mergedTree->u_perp = gjetReader->u_perpZ;
    mergedTree->u_para = gjetReader->u_paraZ;
    mergedTree->triggerFired = gjetReader->triggerFired;

    if (mergedTree->photonPt > 200.0)
      mergedTree->correctEvent = true;
    else
      mergedTree->correctEvent = false;
